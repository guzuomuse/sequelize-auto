...
AutoSequelize.prototype.write = function (attributes, callback) {
    ...
    function createFile(table, _callback) {
        var new_table_name=table;
        if (self.options.prefix) {
            var regex = new RegExp("^" + self.options.prefix);
            new_table_name = table.replace(regex, '');
        }
        fs.writeFile(path.resolve(path.join(self.options.directory, new_table_name + '.js')), attributes[table], _callback);
    }
}
